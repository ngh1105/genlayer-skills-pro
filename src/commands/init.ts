    ...
    if (aiType === 'all') {
      ...
      for (const [ai, paths] of results.entries()) {
        if (paths.length > 0) {
          ...
          for (const p of paths) logger.file(p);
        }
      }
    } else {
      writtenPaths = await installForPlatform(aiType as Exclude<AIType, 'all'>, cwd, options.force ?? false);
      spinner.succeed(`GenLayer skills installed for ${AI_DISPLAY_NAMES[aiType]}!`);
      logger.blank();
      for (const p of writtenPaths) logger.file(p);
    }
