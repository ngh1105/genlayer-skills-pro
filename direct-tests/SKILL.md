---
name: direct-tests
description: Write and run fast in-memory direct mode tests for GenLayer intelligent contracts. Use when user asks to write or run direct mode tests.
---
...
def test_example(direct_vm, direct_deploy, direct_alice, direct_bob):
    # direct_vm       — VMContext with cheatcodes (control blockchain state)
    # direct_deploy   — function to deploy a contract file
    # direct_alice    — test address #1
    # direct_bob      — test address #2
    # direct_charlie  — test address #3
    # direct_owner    — deployer address
    # direct_accounts — list of all test addresses
    pass
```
...
def test_set_and_get(direct_vm, direct_deploy, direct_alice):
    ...
    # 3. Call write methods (state changes work directly)
...
### Full mock format (with headers/method control)
...
direct_vm.mock_llm(
    ...
    json.dumps({"score": "2:1", "winner": 1}),  # Mock response
)
```
...
```python
direct_vm.clear_mocks()  # Reset all mocks between test scenarios
```
...
# Set native token value (wei)
direct_vm.value = 1_000_000_000_000_000_000  # 1 ETH

# Expect a revert with specific message
with direct_vm.expect_revert("Insufficient balance"):
    contract.withdraw(1000)

# Temporary sender change (like Foundry's vm.prank)
with direct_vm.prank(direct_bob):
    contract.method()  # Executes as bob

# Snapshot and restore blockchain state
snap_id = direct_vm.snapshot()
contract.modify_state()
direct_vm.revert(snap_id)   # State restored to snapshot

# Set account balance
direct_vm.deal(direct_alice, 1_000_000_000_000_000_000)
...
### Reusable mock helpers in conftest.py
```python
import json
...
- Always set `direct_vm.sender = ...` before calling any write method
- Run `genvm-lint schema contracts/my_contract.py --json` to understand the contract interface before writing tests
- Direct mode runs leader function only — validator consensus is NOT exercised
- Use integration tests for full consensus validation before deploying

## Next Steps

→ For full consensus testing: see `skills/integration-tests/SKILL.md`
